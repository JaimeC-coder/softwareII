<div class="box box-info padding-1">
    <div class="box-body">
                ...
                <div class="form-group{{ $errors->has('products') ? ' has-error' : '' }}">
                    ...
                    <select id="productos" class="form-control">
                        ...
                        @foreach ($productos as $item)
                            <option
                                value="{{ $item->idProducto }}_{{ $item->pronombre }}_{{ $item->propreciocompra }}_{{ $item->prostock }}_{{ $item->unidadmedida()->first()->umednombre }}">
                                {{ $item->pronombre }}-{{ $item->unidadmedida()->first()->umednombre }}
                            </option>
                        @endforeach
                    </select>
                    {!! $errors->first('idTipocomprobante', '<div class="invalid-feedback">:message</div>') !!}
        ...
        <div class="form-group">
            ...
            {{ Form::text('orcomtotal', $compra->orcomtotal, ['class' => 'form-control' . ($errors->has('orcomtotal') ? ' is-invalid' : ''), 'placeholder' => 'Total']) }}
            {!! $errors->first('orcomtotal', '<div class="invalid-feedback">:message</div>') !!}
        </div>

        <div class="form-group">
            {{ Form::label('orcomdescripcion', 'Observaciones') }}
            {{ Form::text('orcomdescripcion', $compra->orcomdescripcion, ['class' => 'form-control' . ($errors->has('orcomdescripcion') ? ' is-invalid' : ''), 'placeholder' => 'Observaciones']) }}
